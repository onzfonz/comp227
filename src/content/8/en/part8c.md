...
We could ignore these rules but a better idea is to follow the advice the editor gives in the ***Quick Fix*** and set the parameter type to `unknown`:
...
One issue arises after these changes. Our data in file *data/diaries.ts* does not conform to our types anymore:
